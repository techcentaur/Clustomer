...
class Cluster:
    ...
    def get_clusters(self, data):
        ...
        tmp_idx = sorted(sample(range(len(data)), self.n))
        ...
        while not_converged:
            ...
            for i in range(len(prev_clusters)):
                ...
                self.centroids[i] = (
                    (sumx / sumw), sumy / sumw, sumw / len(prev_clusters[i]))
            ...
            diff = 0
            for i in range(len(self.centroids)):
                diff += ((self.centroids[i][0] -
                          self.old_centroids[i][0])**2 +
                         (self.centroids[i][1] -
                          self.old_centroids[i][1])**2)**0.5
...
class Logic:
    ...
    def get_data_frame(self):
        ...
        if query['type'] == 'discrete':
            ...
            return self.df[(self.df[query['col']] >= query['from']) & (
                self.df[query['col']] <= query['to'])]
